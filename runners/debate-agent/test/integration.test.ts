...
test("watch daemon: delegate solo remote_ops launches Claude with default permission mode", async () => {
  const ctx = setup();
  const claudeArgs = join(ctx.root, "claude_solo_remote_args");
  makeStub(
    ctx.binDir,
    "claude",
    "#!/usr/bin/env bash\n" +
  ...
  try {
    ...
    const id = "20260608-itest-delegate-solo-remote-ops";
    writeFileSync(
      join(delegateMailbox, "requests", `${id}.json`),
      JSON.stringify({
        schema_version: 1,
        ...
        repo: realpathSync(ctx.repo),
        provider: "claude",
        capabilities: ["remote_ops"],
        mode: "once",
        task: "TASK-MARKER-SOLO-REMOTE-OPS",
        max_minutes: 1,
      }),
    );
    ...
    assert.ok(!argv.includes("TASK-MARKER-SOLO-REMOTE-OPS"), "delegate task must go through stdin, not argv");
    const config = JSON.parse(readFileSync(join(resp.artifacts_dir, "config.json"), "utf8"));
    assert.deepEqual(config.capabilities, ["remote_ops"]);
...
test("watch daemon: delegate workspace_write + remote_ops launches Claude with static tools and SSH agent env", async () => {
  const ctx = setup();
  ...
  try {
    ...
    assert.doesNotMatch(argv, /bypass/);
    assert.match(argv, /--allowedTools/);
    assert.match(argv, /Bash\(ssh:\*\)/);
    assert.match(argv, /Bash\(scp:\*\)/);
    assert.ok(!argv.includes("TASK-MARKER-REMOTE-OPS"), "delegate task must go through stdin, not argv");
    const config = JSON.parse(readFileSync(join(resp.artifacts_dir, "config.json"), "utf8"));
    assert.deepEqual(config.capabilities, ["workspace_write", "remote_ops"]);
    assert.deepEqual(config.remote_ops, { allowed_bash_patterns: ["ssh:*", "scp:*"], inject_ssh_auth_sock: true });
  } finally {
    if (daemon?.pid !== undefined) {
      try {
